...
```
# Starting minikube
In order to start using kubernetes we need to create a cluster. The simplest approach is to use minikube. In order to create a minikube cluster we use the commands below:
- `minikube start --vm-driver=hyperkit` - to create minikube
- `minikube delete` - will delete the existing minikube
- `minikube start --driver=docker` - Start the existing minikube cluster
...
# Yaml configuration
There are three parts of a K8s configuration file.
- Metadata, this includes the name
- Specification, this includes the configuration you want to apply to the component, you include what you want to create (kind), the attributes under spec will depend on what type of service it is.
- Status, this is automatically generated by kubernetes, it will compare the desired and actual state of the pod, so it will trigger the self healing. The information is taken from etcd.

The files for configuration is written in yaml. Indentation is very important, you can use yaml validator to check it.

Labels and selectors are used to link services to deployment.
